...
import { faCalendar, faCircleInfo, faClosedCaptioning, faMicrophone, faStar } from '@fortawesome/free-solid-svg-icons';
  ...
  return (
    ...
    >
      {trendingAnime?.map((anime, index) => {
        const animeTitle = anime?.name || anime?.jname || '';
        return (<SwiperSlide key={anime.id} className='relative'>
          <div className={`image-overlay w-full h-full relative`}>
            <Image src={anime.poster ?? ''} alt={anime.id} fill sizes='100vw' className='object-cover w-full blur-md' style={{objectPosition: '50% 20%'}} />
          </div>
          <section className='slider-content flex flex-col gap-2 items-start absolute w-[200px] md:w-[300px] lg:w-[400px] top-[20%] md:top-[10%] lg:top-[20%] left-[10%]'>
            <p className='uppercase text-xs lg:text-lg font-semibold text-yellow'>Spotlight #{index+1}</p>
            ...
            <div className='flex flex-wrap items-center gap-1 text-xs md:text-sm lg:text-sm font-semibold'>
              {/* <p className='text-yellow'><FontAwesomeIcon icon={faStar} /> {anime.rating ? `${(anime.rating / 10).toFixed(1)}` : 'No ratings'}</p> */}
              <p><FontAwesomeIcon className='text-yellow' icon={faCalendar}/> {anime.otherInfo[2]}</p> •
              {/* <p>{anime.episodes.sub ? `Episodes: ${anime.episodes.sub} •` : ''}</p> */}
              <p className='border border-white px-1 rounded-sm flex items-center gap-1'>
                <FontAwesomeIcon icon={faClosedCaptioning} className='mr-1' />
                {anime.episodes.sub ? `${anime.episodes.sub}` : 'No subs'}
              </p>
              <p className='border border-white px-1 rounded-sm'>
                <FontAwesomeIcon icon={faMicrophone} className='mr-1' />
                {anime.episodes.sub ? `${anime.episodes.dub}` : 'No Dubs'}
              </p>
          ...
          <div className='absolute top-[-10%] right-[-30px] md:right-0 lg:right-[10%]'>
            <div className='relative rotate-12 scale-125 w-[200px] h-[300px] lg:w-[300px] lg:h-[500px] border-white border-[10px]'>
              <Image src={anime.poster ?? ''} alt={animeTitle} fill sizes='100%' className='object-cover '/>
            </div>
          </div>
